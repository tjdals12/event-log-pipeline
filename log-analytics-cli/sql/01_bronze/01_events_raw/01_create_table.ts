...
export const buildQuery = (args: {
  ...
  const { bronzeDb, bucket } = args;
  const query = sql`
    CREATE EXTERNAL TABLE IF NOT EXISTS \`${sql.raw(bronzeDb)}\`.events_raw (
        event_uuid string,
        event_name string,
    ...
    STORED AS TEXTFILE
    LOCATION 's3://${sql.raw(bucket)}/bronze/raw/events/'
    TBLPROPERTIES (
        ...
        = 's3://${sql.raw(
          bucket
        )}/bronze/raw/events/year=\${year}/month=\${month}/day=\${day}/'
    )
  `;

  const complied = query.compile(bronze).sql;
