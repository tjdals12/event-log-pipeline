...
const command = new Command("bronze");

command
  .description("브론즈 레이어(원천 데이터) 관련 작업을 수행합니다.")
  .helpOption("-h, --help", "자세한 도움말 보기")
  .showHelpAfterError(
    "예: analytics bronze --stage dev --dataset events_raw --action create-table"
  )
  .requiredOption("--stage <name>", "환경. 예: dev, prod")
  .requiredOption("--dataset <dataset>", "대상. 예: events_raw")
...
command.configureHelp({
  formatHelp: (command, helper) => {
    ...
    const availableValues = [
      ...
      `${indent(8)}${"--dataset".padEnd(10)}: ${Object.keys(BRONZE.DATASETS)}`,
    ]
      .map((l) => t.dim(l))
      .join("\n");
    ...
    const datasets = Object.keys(
      BRONZE.DATASETS
    ) as (keyof typeof BRONZE.DATASETS)[];
    const actionsByDataset = datasets
      .map((dataset) => {
        const { actions } = BRONZE.DATASETS[dataset];
    ...
    const datasetDetails = datasets
      .map((dataset) => {
        const { description, actions } = BRONZE.DATASETS[dataset];
        return [
          `${indent(8)}- ${dataset}`,
    ...
    const examples = [
      `${t.example("analytics")} bronze ${t.dim(
        "--stage dev --dataset events_raw --action create-table"
      )}`,
      `${t.example("analytics")} bronze ${t.dim(
        "--stage prod --dataset events_raw --action create-table"
      )}`,
    ]
      .map((l) => indent(8) + "$ " + l)
      .join("\n");
