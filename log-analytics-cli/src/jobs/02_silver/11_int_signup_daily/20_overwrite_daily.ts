...
import { buildQuery as buildCreateTempTableQuery } from "@/sql/02_silver/11_int_signup_daily/20_CTAS_daily_versioned";
import { buildQuery as buildDropPartitionQuery } from "@/sql/02_silver/11_int_signup_daily/21_drop_partition";
import { buildQuery as buildAddPartitionQuery } from "@/sql/02_silver/11_int_signup_daily/22_add_partition";
import { buildQuery as buildDropTempTableQuery } from "@/sql/02_silver/11_int_signup_daily/23_drop_tmp_table";
...
const execute = async (config: Config, args?: unknown) => {
  ...
  const createTempTableSqlPath =
    "sql/02_silver/11_int_signup_daily/20_CTAS_daily_versioned.ts";
  const dropPartitionSqlPath =
    "sql/02_silver/11_int_signup_daily/21_drop_partition.ts";
  const addPartitionSqlPath =
    "sql/02_silver/11_int_signup_daily/22_add_partition.ts";
  const dropTempTableSqlPath =
    "sql/02_silver/11_int_signup_daily/23_drop_tmp_table.ts";
