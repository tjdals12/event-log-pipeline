...
import { buildQuery } from "@/sql/02_silver/20_int_active_daily/90_drop_table";
...
const execute = async (config: Config, args: unknown): Promise<void> => {
  ...
  const job = "Drop Table · silver/int_active_daily";
  const sqlPath = "sql/02_silver/20_int_active_daily/90_drop_table.sql";
