...
import { buildQuery as buildDropPartitionQuery } from "@/sql/02_silver/21_int_first_session_after_signup/10_drop_partition";
import { buildQuery as buildInsertDailyQuery } from "@/sql/02_silver/21_int_first_session_after_signup/11_insert_daily";
...
const execute = async (config: Config, args?: unknown): Promise<void> => {
  ...
  const dropPartitionSqlPath = `sql/02_silver/21_int_first_session_after_signup/10_drop_partition.sql`;
  const insertDailySqlPath = `sql/02_silver/21_int_first_session_after_signup/11_insert_daily.sql`;
