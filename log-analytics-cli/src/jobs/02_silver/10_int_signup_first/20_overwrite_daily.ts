...
import { buildQuery as buildCreateTempTableQuery } from "@/sql/02_silver/10_int_signup_first/20_CTAS_daily_versioned";
import { buildQuery as buildDropPartitionQuery } from "@/sql/02_silver/10_int_signup_first/21_drop_partition";
import { buildQuery as buildAddPartitionQuery } from "@/sql/02_silver/10_int_signup_first/22_add_partition";
import { buildQuery as buildDropTempTableQuery } from "@/sql/02_silver/10_int_signup_first/23_drop_tmp_table";
...
const execute = async (config: Config, args?: unknown) => {
  ...
  const job = `Daily Partition Overwrite · silver/int_signup_first · ${year}/${month}/${day}`;
  const createTempTableSqlPath =
    "sql/02_silver/10_int_signup_first/20_CTAS_daily_versioned.ts";
  const dropPartitionSqlPath =
    "sql/02_silver/10_int_signup_first/21_drop_partition.ts";
  const addPartitionSqlPath =
    "sql/02_silver/10_int_signup_first/22_add_partition.ts";
  const dropTempTableSqlPath =
    "sql/02_silver/10_int_signup_first/23_drop_tmp_table.ts";
